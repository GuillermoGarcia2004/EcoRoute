...
export const ListarRecompensas = () => {
  ...
  const listarRecompensas = async () => {
    try {
      const response = await fetch(`http://localhost:3000/api/v1/ecoroute/recompensas/${obtenerIdUsuario()}`);
      if (!response.ok) {
  ...
  const canjearRecompensa = async (recompensa) => {
    try {
      ...
      if (!id_usuario) throw new Error("No se encontró el ID del usuario.");
      const payload = {
        id_usuario,
        id_recompensa: Number(recompensa.id),
      };
      const response = await fetch(
        "http://localhost:3000/api/v1/ecoroute/recompensas/canjear",
        {
          method: "POST",
      ...
      if (!response.ok || data.error) {
        setMensajeError(data.error || "Error al canjear la recompensa");
        setMostrarError(true);
        setTimeout(() => {
          setMensajeError("");
          setMostrarError(false);
      ...
      setMensajeExito(data.mensaje || "Recompensa canjeada con éxito");
      ...
      console.log(error);
      setMensajeError(error.message || "Error al canjear la recompensa");
      setMostrarError(true);
      setTimeout(() => {
        setMensajeError("");
        setMostrarError(false);
  ...
  return (
    <UserView>
      <div className="mb-5">
        <h1 className="text-white text-2xl">Recompensas adquiridas:</h1>
        {mostrarExito && (
      ...
      {recompensas.length === 0 ? (
        <div className="text-white text-lg text-center mt-10">El usuario todavía no ha adquirido recompensas.</div>
      ) : (
        <div className="w-[80%] h-[500px] p-5 overflow-y-auto [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:bg-black/80 [&::-webkit-scrollbar-thumb]:rounded-full grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {recompensas.map((r) => (
            <div key={r.id} className="bg-white border rounded-lg p-4 shadow-[11px_10px_5px_0px_rgba(0,0,0,0.75)] h-[220px] flex flex-col justify-between">
              ...
              <button onClick={() => canjearRecompensa(r)} className="w-full bg-green-600 hover:bg-green-700 text-white font-bold py-2 px-4 rounded-lg transition-colors duration-200 shadow-md cursor-pointer">CANJEAR RECOMPENSA</button>
            </div>
          ))}
        </div>
      )}
